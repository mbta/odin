...
def migration() -> None:
    ...
    log = ProcessLog(
        "odin_migration", migration="alpha_prod_0005", target_tables=", ".join(target_tables)
    )
    target_paths = [os.path.join(CUBIC_ODS_FACT_STATUS, x) + ".json" for x in target_tables]

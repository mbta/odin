...
FROM cubic_ods.edw_payment_summary p
	...
	LEFT JOIN cubic_ods.edw_reason_dimension r ON r.reason_key = p.reason_key
...
FAREREV_REFUND_SUMMARY
	JOIN cubic_ods.edw_fare_revenue_report_schedule FAREREV_REPORT_SCHEDULE ON FAREREV_REFUND_SUMMARY.OPERATING_DAY_KEY = FAREREV_REPORT_SCHEDULE.COMP_OPERATING_DAY_KEY
	RIGHT JOIN cubic_ods.edw_payment_type_dimension PAYMENT_TYPE_DIMENSION ON FAREREV_REFUND_SUMMARY.PAYMENT_TYPE_KEY = PAYMENT_TYPE_DIMENSION.PAYMENT_TYPE_KEY
	JOIN cubic_ods.edw_date_dimension OPERATING_DATE_DIMENSION ON OPERATING_DATE_DIMENSION.DATE_KEY=FAREREV_REFUND_SUMMARY.OPERATING_DAY_KEY
	JOIN cubic_ods.edw_date_dimension SETTLEMENT_DATE_DIMENSION ON SETTLEMENT_DATE_DIMENSION.DATE_KEY=FAREREV_REFUND_SUMMARY.SETTLEMENT_DAY_KEY
GROUP BY
'WC700',
operating_day,
settlement_day,
